...
void rateinmaze_helper(int **maze,int n,int **solution,int x,int y){
    // Base case
    if(x==n-1 and y==n-1){
        solution[x][y]=1;
    ...
    if(x<0 or x>=n or y<0 or y>=n or maze[x][y]==0 or solution[x][y]==1){
        return;
    }
    solution[x][y]=1;
    rateinmaze_helper(maze,n,solution,x-1,y);
    rateinmaze_helper(maze,n,solution,x+1,y);
    rateinmaze_helper(maze,n,solution,x,y-1);
    rateinmaze_helper(maze,n,solution,x,y+1);
    solution[x][y]=0;
}
void rateinmaze(int** maze,int n){
    int **solution=new int*[n];
    for(int i=0; i<n; i++){
        solution[i]=new int[n];
        for(int j=0; j<n; j++){
            solution[i][j]=0;  // initially intialise out solution matrix with 0
        }
    }
    rateinmaze_helper(maze,n,solution,0,0);
}
int main(){
    int n; cin>>n;
    int **maze=new int*[n];
