...
bool checkpathhelper(int **edges,int n,int sv,int v2,bool *visited){
    if(edges[sv][v2]=1){
        return true;
    }
    visited[sv]=true;
    for(int i=0; i<n; i++){
        if(edges[sv][i]==1 and !visited[i]){
            if(checkpathhelper(edges,n,i,v2,visited)){
                return true;
            }
        }
    }
...
bool checkpath(int **edges,int n,int v1,int v2){
    bool *visited=new bool[n];
    for(int i=0; i<n; i++){
        visited[i]=false;
    }
    return checkpathhelper(edges,n,v1,v2,visited);
}
...
int main() {
    ...
    int v1,v2; cin>>v1>>v2;
    // (checkpath(edges,n,v1,v2))?"true":"false";
    if(checkpath(edges,n,v1,v2)){
        cout<<"true";
    }else{
        cout<<"false";
    }
